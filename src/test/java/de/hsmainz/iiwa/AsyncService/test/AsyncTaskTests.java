...
public class AsyncTaskTests {
    ...
    public void async_task_fire() {
        ...
        AsyncSupplier<Integer> suppi = Async.makeAsync(() -> { return 33;});
    ...
    @Test
    public void async_convenience_class() {

        counter = 0;

        AsyncService.init();

        Async.run(() -> { System.out.println("Hello from Runnable!"); counter++; });
        Async.run((i) -> { System.out.println("Hello from Consumer!"); counter++; });
        Async.run(() -> { System.out.println("Hello from Supplier!"); counter++; return 3; });
        Async.run(() -> { System.out.println("Hello from Function!"); counter++; });
        Async.run((k,v) -> { System.out.println("Hello from BiConsumer!"); counter++; });
        Async.run((l,j) -> { System.out.println("Hello from BiFunction!"); counter++; return 40; });
    ...
    public void strand_test() {
        ...
        try{
            executor.post(() -> {
                ...
                try {
                    ...
                    executor.post(() -> { legal_exec_2 = true; });
        ...
        try{
            executor.post(() -> { System.out.println("hello"); });
        } catch (ExecutionRejectedException ex){
    ...
    public void rate_limiter_test(){
        ...
        RateLimitedExecutor executor = new RateLimitedExecutor(100);

        AsyncTask poster = Async.makeAsync(() -> {
        ...
        AsyncService.scheduleInterval(poster, 10);
        AsyncService.schedule(poster.getFuture().getTimeout(), 1000);
